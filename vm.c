  ...
  for(; a  < oldsz; a += PGSIZE){
    ...
    if(!pte)
      ...
      if(pa == 0)
        panic("kfree");
      char *v = P2V(pa);
      decrementRcount(pa, myproc()->pid);					//decrease reference count
...
cowuvm(struct proc* p, uint sz)
{
  ...
  for(i = 0; i < sz; i += PGSIZE){
    ...
    if((*pte & PTE_P)){
      ...
      int blockid = *(walkpgdir(p->pgdir,(char*)i,0))>>12;     //disk id where the page was swapped
    ...
    *pte &= ~PTE_W;						//change the write permission to read only
    ...
    if(mappages(d, (void*)i, PGSIZE, pa, flags) < 0)		//map the same pages to child process page table
      goto bad;
  	incrementRcount(pa, p->pid);
  	//if(p->parent->rss > p->rss) 
    // if(p->pid>2) p->rss-=PGSIZE;
    //p->rss+=PGSIZE;
    lcr3(V2P(p->pgdir));					//reinstall page table as all entries flags are changed
...
void pagefault(uint err_code)						//page fault handling function
{
  ...
  if(!(*pte & PTE_P))        
  {
    if(myproc()->pid>2){
      ...
      addr &= ~0xfff;
      map_address(myproc(), addr);
      pa = PTE_ADDR(*pte);
      incrementRcount(pa, myproc()->pid);
      return;
    }
  ...
  if(getRcount(pa) == 1){							//only need to change the page table entries	
    *pte = *pte | PTE_W;
    ...
    decrementRcount(pa, myproc()->pid);								//decrement reference count
    //cprintf("%d\n", myproc()->pid);
    setRcount(V2P(mem), 1);
...
void free_swap_Space_on_exit(pde_t * pgdir)
{
  ...
  {
    prob_victim = walkpgdir(pgdir, (void*)uviraddr, 0); 
     ...
     if ( (((*prob_victim) & PTE_P) == 0)   &&  (((*prob_victim)&PTE_SWAPPED) == PTE_SWAPPED))
     {
        int bno = *(walkpgdir(pgdir,(char*)uviraddr,0))>>12;
        //cprintf("%d\n",bno);
